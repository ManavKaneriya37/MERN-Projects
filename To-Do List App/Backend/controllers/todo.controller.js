...
module.exports.createTodo = async (req, res) => {
  try {
    ...
    if (!title || !description || !user)
      return res
        .status(400)
        .json({ message: "Either Title or description or user id is missing" });
...
module.exports.getPriorityTodos = async (req, res) => {
  ...
  const allPriorityTodos = await todoModel.find({
    user: userid,
    priority: { $ne: "None" },
  });
...
module.exports.getTodaysTodos = async (req, res) => {
  ...
  const allTodaysTodos = await todoModel.find({
    user: userid,
    date: todaysDate,
  });
...
module.exports.updateData = async (req, res) => {
  try {
    const { _id, title, description, date, priority, category } =
      req.body.formData;
...
module.exports.filterData = async (req, res) => {
    try {
        ...
        if(category !== 'Default') {
            const filteredTodos = await todoModel.find({category});
            res.status(200).json(filteredTodos);
        } else {
            const user = req.user;
            console.log(user);

            const allTodos = await todoModel.find({user: user._id});
