...
const createProject = asyncHandler(async (req, res) => {
    try {
        const {name, description, userId, budget} = req.body;
...
const updateProject = asyncHandler(async ( req, res ) => {
    try {
        ...
        .status(200)
        .json(
            new ApiResponse(200, null, "Project details updated successfully")
        )
...
const deleteProject = asyncHandler(async (req, res) => {
    try {
        ...
        const getProject = await ProjectModel.findById(deletedProject._id.toString());
        if(getProject) {
